...
from multiremodnav.MultiRemodnavWorker import MultiRemodnavWorker
import sys, glob, re
...
class MultiRemodnavWindow(QWidget):
    total = 0
    done = 0
    ...
    def initUI(self):
        self.setWindowTitle('Multi remodnav')

        self.resize(502, 278)

        self.fetchButton = QPushButton('Fetch files to remodnav', self)
        self.fetchButton.setGeometry(QRect(20, 20, 181, 26))
        self.fetchButton.clicked.connect(self.fetchFilesToAnalyse)

        self.analyseButton = QPushButton('Start remodnav', self)
    ...
    def analyseFiles(self):
        ...
        for i in range(count):
            ...
            analyse_worker = MultiRemodnavWorker(row, self.batchIDInput.text())
