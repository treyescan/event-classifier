...
def main(args=sys.argv):
    ...
    # Fix for later versions of Python (getargspec was deprecated)
    ...
    clf = EyegazeClassifier(
        **{k: getattr(args, k) for k in (
            'sampling_rate', 'velthresh_startvelocity',
    ...
    clf.show_gaze(pp=pp, events=events, show_vels=True)
    pl.xlim((0, duration))
    pl.xticks(np.arange(0, duration, step=1))
    title = 'Detected eye movement events, parameters: {}'.format(
        ', '.join([
    ...
    pl.yticks([-2880, -1000, 0, 1000, 2880])
    pl.xlabel('time (seconds)')

    # set axes limits
    ax = pl.gca()
    ax.set_ylim([-2880, 2880])

    basename = args.outfile[:-4] if args.outfile.endswith('.tsv') else args.outfile
    pl.savefig(
        '{}.png'.format(basename),
        bbox_inches='tight', format='png', dpi=100, pad_inches=0)
